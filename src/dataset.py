        ...
        if method == "byUser":
            ...
            for (uid, items) in user_item_map.items():
                ...
                if n_items_u >= 3 and n_vali_samples < max_validation_test_samples:
                    ...
                    val_set.append([uid, items[vali_test_index[0], 0]])
                    test_set.append([uid, items[vali_test_index[1], 0]])
                    for idx in range(n_items_u):
                        if idx not in vali_test_index:
                            train_set.append([uid, items[idx, 0]])
                else:
                    for idx in range(n_items_u):
                        train_set.append([uid, items[idx, 0]])
            ...
            print("Successfully split data in to train/validation/test!")

            train_df = pd.DataFrame(np.array(train_set, dtype=int),
                                    columns=["user_number", "book_number"])
            train_df.to_csv(DATA_DIR + self.DATA_NAME + "_train.csv", index=False)
            val_df = pd.DataFrame(np.array(val_set, dtype=int),
                                  columns=["user_number", "book_number"])
            val_df.to_csv(DATA_DIR + self.DATA_NAME + "_val.csv", index=False)
            test_df = pd.DataFrame(np.array(test_set, dtype=int),
                                   columns=["user_number", "book_number"])
            test_df.to_csv(DATA_DIR + self.DATA_NAME + "_test.csv", index=False)
